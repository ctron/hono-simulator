...
public abstract class OkHttpDevice extends Device {
    ...
    protected ThrowingSupplier<CompletableFuture<?>, Exception> tickTelemetryProvider() {
        return () -> doPublish(this::createTelemetryCall);
    }

    @Override
    protected ThrowingSupplier<CompletableFuture<?>, Exception> tickEventProvider() {
        return () -> doPublish(this::createEventCall);
    }

    protected abstract CompletableFuture<?> doPublish(final Supplier<Call> call) throws Exception;
